...
function DisasmInstruction( byref uAddr as long ) as bool
  ...
  color 7 : printf("%04X: ",uAddr)
  color 8 : printf("%02 ",bOP)
  
  uAddr += 1 
  if iLen=0 then 
    locate ,16 : color 12 : puts("ILLEGAL")
    return false
  end if
  ...
  case cvl("xxxx") '16bit absolute (direct or indexed)
    bDig = 4 : bSz = 2
  case else        '>>> Error <<<        
    color 12 : printf("{%s}%08X",uOp): sleep
  end select
  
  if bSz=1 then 
    print hex( cint(g_bMemory(uAddr)) , 2 );
    if bDig = 2 then uVal = g_bMemory(uAddr)
  end if
  if bSz=2 then 
    print hex( cint(g_bMemory(uAddr+0)) , 2 );" ";
    print hex( cint(g_bMemory(uAddr+1)) , 2 );
    uVal = Read16(uAddr)
  end if
  uAddr += bSz      
  ...
  locate ,16 : color 10 : printf("%s",left(sOpcode,4))
      ...
      dim as ulong uAddr = g_tBlock(N).uBegin , uAddrEnd = g_tBlock(N).uEnd , uBytes
        ...
        do
          ...
          if uBytes >= 4 orelse uAddr >= uAddrEnd orelse iLen then
            sOpcode += "      "
            if uBytes then        
              color 7 : print hex(cint(uAddr),4);": "; : color 8
              for N1 as long = uAddr-uBytes to uAddr-1
                print hex(cint(g_bMemory(N1)),2);" ";
              next N1
            ...
            color 7 : print hex(cint(uAddr-1),4);": ";
            color 8 : print hex(cint( bOP   ),2);" ";
            ...
            case cvl("xxxx") '16bit absolute (direct or indexed)
              bDig = 4 : bSz = 2
            case else        '>>> Error <<<        
              color 12 : print "{"+sOpcode+"}",hex(cint(uOp),8): sleep
            end select
