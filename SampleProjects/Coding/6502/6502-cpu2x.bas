namespace cpu
  ...
  type CpuCore
    ...
    #ifdef LazyFlagsNZ
      as ulong iLFResuNZ, iLfResuC, iLfResuV
    #endif
  end type  
  ...
  rem ------ cpu read/write/notify helpers -----
    #ifdef CpuNotifyRead
  ...
  rem -------------- flags macros --------------
    ...
    #ifdef LazyFlagsC
      #define GetFlag_Carry() ((.iLfResuC shr 8) and 1)
    #else
      #define GetFlag_Carry() GetFlag(Carry)
    #endif
  ...
  'opcode list (used to generate all opcode functions)
  ...
  rem ---------------- flags ------------
    ...
    #endmacro
    #macro fnOpcode_SEI() 'Set Interrupt
  ...
  rem --------------  memory ------------
    ...
    #macro fnOpcode_BIT() 'Bit Test memory
      ...
      'The N and V flags are set to match bits 7 and 6 respectively in the value stored at the tested address.
        ...
        #ifdef LazyFlagsV
          .iLfResuV = ((.iLFResuNZ shl 2) and 256)
        #else
  ...
  rem ---------------- math -------------
    #macro fnOpcode_ADC() 'ADC A with memory or imm
      ...
      #ifdef LazyFlagsNZ
        .iLFResuNZ = .RegA + ReadByteOpcode(wMemAddr) + GetFlag_Carry()
        .RegA = .iLFResuNZ
        #ifdef LazyFlagsC         
          .iLfResuC = .iLFResuNZ
        #else
    ...
    #macro fnOpcode_SBC() 'SBC A with memory or imm
      ...
      #ifdef LazyFlagsNZ
        .iLFResuNZ = .RegA - (ReadByteOpcode(wMemAddr) + (1-GetFlag_Carry()))
        .RegA = .iLFResuNZ
        #ifdef LazyFlagsC
          .iLfResuC = not .iLFResuNZ
        #else
  ...
  rem --------------- branch ------------
    #macro fnOpcode_BPL() 'Branch if Negative Clear (plus)
      ...
      #define wMemAddr ((.RegPC+cbyte(ReadByteFast(.RegPC))) and &hFFFF)      
      #ifdef LazyFlagsNZ
    ...
    #macro fnOpcode_BCC() 'Branch if Carry Clear
      ...
      #define wMemAddr ((.RegPC+cbyte(ReadByteFast(.RegPC))) and &hFFFF)      
      #ifdef LazyFlagsC
        if .iLfResuC <= 255 then .RegPC = wMemAddr
      #else
  ...
  rem --------------- shift -------------
    ...
    #macro fnOpcode_LSRA() 'Shift Right Accumulator
      ...
      'and the original bit 0 is shifted into the Carry.
        ...
        #ifdef LazyFlagsC
          .iLfResuC = culng(.RegA and 1) shl 8
        #else
    ...
    #macro fnOpcode_LSR() 'Shift Right
      ...
      'and the original bit 0 is shifted into the Carry.
        ...
        .iLFResuNZ = ReadByteOpcode(wMemAddr)
        #ifdef LazyFlagsC
          .iLfResuC = culng(.iLFResuNZ and 1) shl 8
        #else
    ...
    #macro fnOpcode_RORA() 'Rotate Right (trough carry) Accumulator
      ...
      #ifdef LazyFlagsNZ
        .iLFResuNZ = (.RegA shr 1) or (GetFlag_Carry() shl 7)
        #ifdef LazyFlagsC
          .iLfResuC = culng(.RegA And 1) shl 8
        #else
          GetFlag(Carry) = (.RegA and 1)
    ...
    #macro fnOpcode_ROR() 'Rotate Right (trough carry)
      ...
      'and the original bit 0 is shifted into the Carry.
        ...
        .iLFResuNZ = (Src shr 1) or (GetFlag_Carry() shl 7)
        WriteByteOpcode(wMemAddr,.iLFResuNZ)
        #ifdef LazyFlagsC
          .iLfResuC = culng(Src and 1) shl 8
        #else
          GetFlag(Carry) = (Src and 1)
        #endif
      #else
    ...
    #macro fnOpcode_ROLA() 'Rotate Left (trough carry) accumulator
      ...
      #ifdef LazyFlagsNZ
        .iLFResuNZ = (culng(.RegA) shl 1) or GetFlag_Carry()
        .RegA = .iLFResuNZ
        #ifdef LazyFlagsC
          .iLfResuC = .iLFResuNZ
        #else
    ...
    #macro fnOpcode_ROL() 'Rotate Left (trough carry)
      ...
      #ifdef LazyFlagsNZ
        .iLFResuNZ = (culng(ReadByteOpcode(wMemAddr)) shl 1) or GetFlag_Carry()
        WriteByteOpcode(wMemAddr,.iLFResuNZ)
        #ifdef LazyFlagsC
          .iLfResuC = .iLFResuNZ
        #else
  ...
  rem --------------- stack -------------
    ...
    #macro fnOpcode_PLP() 'Pull Processor status (flags)
      ...
      .RegP.bFlags = ReadByteFast((&h100+.RegSP))      
        ...
        #ifdef LazyFlagsC
          .iLfResuC = FlagVar(Carry) shl 8
        #endif
        #ifdef LazyFlagsV
          .iLfResuV = FlagVar(oVerflow) shl 8 
        #endif
      #endif
      .RegSP += 1
