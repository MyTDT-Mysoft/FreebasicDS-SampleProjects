#define fbc -s console -gen gcc -O 2 -asm intel
...
dim shared as SubtitleStruct tSubTitle(15)
...
dim shared as short CurPal(767),AtuPal(767),CurPal2(767)
dim shared as short DeltaPal(767),StoredPal(767)
...
SanDir = exepath+"/NitroFilesF/" 'exepath+"/"'"h:\fromdata\games4\fullt\VIDEO\"
#ifndef NewFile
dim shared as string NewFile
...
SanDir = "H:/FromData/Games4/Fullt/"
#endif
              ...
              for Y as integer = 0 to .hei-1 'to -1 step -1
                ...
                while LineLen > 0
                  ...
                  if (uCode and 1) then                   
                    ...
                    #ifdef __FB_NDS__
                    if Pix then FastSet(pBuff,Pix,uCode)
                    #else
                ...
                case 2         'RLE
                  ...
                  while cuint(DataPtr) < cuint(DataEnd)
                    ...
                    if (uCode and 1) then                    
                      uCode = (uCode shr 1)+1
                      #ifdef __FB_NDS__
                      FastSet(pBuff,*cptr(ubyte ptr,DataPtr+1),uCode)
                      #else
